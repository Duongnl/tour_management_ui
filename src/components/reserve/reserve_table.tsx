...
const ReserveTable = (props: IProps) => {
  ...
  const [reserveTours, setReserveTours] = useState<IReserveTourResponse[]>(
    props.reserveTours
  );

  const [reserveToursFilter, setReserveToursFilter] = useState<
    IReserveTourResponse[]
  >(props.reserveTours);

  // tổng số trang
  const [numberPages, setNumberPages] = useState<number>(
    Math.ceil(reserveTours != undefined ? reserveTours.length / 3 : 0)
  );
  ...
  const [startDay, setStartDay] = useState<string>(
    startDayParams == null ? "" : startDayParams
  );
  const [endDay, setEndDay] = useState<string>(
    endDayParams == null ? "" : endDayParams
  );
  const [dayFilter, setDayFilter] = useState<boolean>(
    dayFilterParams == "false" || dayFilterParams == "true"
      ? dayFilterParams == "false"
        ? false
        : true
      : false
  );
  const [categorySlug, setCategorySlug] = useState<string>(
    categoryParams == null ? "" : categoryParams
  );
  ...
  const fetchFilterReserveTours = async (
    initDataFilter: IReserveTourFilterRequest
  ) => {
  ...
  useEffect(() => {
    router.push(
      `${pathname}?category=${categorySlug}&&dayFilter=${dayFilter}&&startDay=${startDay}&&endDay=${endDay}`
    );
  ...
  return (
    <>
      <div className="div-filter mb-4 d-flex flex-wrap justify-content-start">
        <InputGroup className=" select-date my-1">
          <InputGroup.Checkbox
            aria-label="Checkbox for following text input "
            checked={dayFilter}
            onChange={(e) => {
              handleSetDayFilter(e.target.checked);
            }}
          />

          <FloatingLabel
            className="float-lable-input width-primary"
            label="Ngày bắt đầu"
          >
            <Form.Control
        ...
        <Form.Select
          ...
          className="select-category width-primary m-1 height-primary"
        ...
        >
          ...
          {categories?.map((category, index) => (
            <>
              <option value={`${category.url}-${category.category_id}`}>
        ...
        <InputGroup className="input-search-reserve-tour  width-primary m-1 height-primary">
      ...
      <div className="table-wrapper">
        <Table striped bordered hover className="table-reserve">
          <thead>
            <tr>
              <th>Tour</th>
              <th>Số chổ</th>
                    ...
                    <tr key={index}>
                      <td>
                        ...
                        <span className="tour-name">
                          {reserveTour.tour_name}
                        </span>
                      </td>
                      <td>
                        <div className="div-text">
                          <i
                            className="fa-solid fa-user-plus icon-reserve"
                            style={{ color: "blue" }}
                          ></i>{" "}
                          <span style={{ color: "#337ab7" }}>
                            {" "}
                            {`Đã bán: ${reserveTour.tourTime.quantity_sell}`}{" "}
                          </span>
                          <br />
                        </div>
                        <div className="div-text">
                          ...
                          <span style={{ color: "#337ab7" }}>
                            {" "}
                            {`Giữ chổ: ${reserveTour.tourTime.quantity_reserve}`}{" "}
                          </span>
                          <br />
                        </div>
                        <div className="div-text">
                          ...
                          <span style={{ color: "green" }}>
                            {`Còn lại: ${reserveTour.tourTime.quantity_left}`}{" "}
                          </span>{" "}
                          <br />
                        </div>
                        <i
                        ...
                        <span
                          style={{ color: "red" }}
                        >{`Hạn visa: ${reserveTour.tourTime.visa_expire}`}</span>
                      </td>
                      <td>
                        <div className="div-text">
                          <i
                          ...
                          <span className="text-schedule">
                            {" "}
                            {`Đi: ${reserveTour.tourTime.departure_date}`}
                          </span>{" "}
                          <br />
                        </div>
                        <i
                        ...
                        <span className="text-schedule">
                          {" "}
                          {`Về: ${reserveTour.tourTime.return_date}`}{" "}
                        </span>
                      </td>
                      <td>
                        <div className="div-text">
                          ...
                          <span className="text-schedule">
                            {" "}
                            {`Đi:  ${reserveTour.tourTime.departureAirline.airline_name} ${reserveTour.tourTime.departureAirline.airline_detail} ${reserveTour.tourTime.departure_time}`}{" "}
                          </span>{" "}
                          <br />
                        </div>
                        <i
                        ...
                        <span className="text-schedule">
                          {" "}
                          {`Về:  ${reserveTour.tourTime.returnAirline.airline_name} ${reserveTour.tourTime.returnAirline.airline_detail} ${reserveTour.tourTime.return_time}`}{" "}
                        </span>
                      </td>
                      <td>
                        <span
                        ...
                        >
                          {`${formatCurrency(reserveTour.tourTime.price_min)}`}{" "}
                        </span>
                      </td>
                      <td>
                        {/* <Button
                            ...
                            >
                              <Link
                                href={
                                  "/management/reserve/" +
                                  CreateSlug(
                                    `${reserveTour.tourTime.time_name} ${reserveTour.tourTime.tour_time_id}`
                                  )
                                }
